...
func (h *handler) handleCallMsg(ctx *callProc, reqCtx context.Context, msg *jsonrpcMessage) *jsonrpcMessage {
	...
	switch {
	case msg.isNotification():
		log.Info("消息类型为通知，开始处理通知请求")
		h.handleCall(ctx, msg)
		h.log.Debug("Served "+msg.Method, "duration", time.Since(start))
		return nil

	case msg.isCall():
		log.Info("消息类型为请求，开始处理请求")
...
func (h *handler) handleCall(cp *callProc, msg *jsonrpcMessage) *jsonrpcMessage {
	if msg.isSubscribe() {
		log.Info("消息为订阅消息，开始处理订阅请求")
		return h.handleSubscribe(cp, msg)
	}
	var callb *callback
	if msg.isUnsubscribe() {
		log.Info("消息为取消订阅消息，直接从handler获取要执行的方法")
		callb = h.unsubscribeCb
	} else {
		log.Info("消息为服务方法消息，开始获取服务方法命名空间及方法名")
	...
	log.Info("开始解析方法参数，并返回方法参数的反射值类型")
